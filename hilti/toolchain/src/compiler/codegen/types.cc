...
struct VisitorDeclaration2 : public type::VisitorBase {
    std::optional<cxx::declaration::Type> _result;

    VisitorDeclaration2(CodeGen* cg, util::Cache<cxx::ID, cxx::declaration::Type>* cache) : cg(cg), cache(cache) {}
...
struct VisitorTypeInfoDynamic : hilti::visitor::PreOrder<cxx::Expression, VisitorTypeInfoDynamic> {
    ...
    result_t operator()(const type::map::Iterator& n) {
        return fmt(
            "::hilti::rt::type_info::MapIterator(%s, %s, ::hilti::rt::type_info::MapIterator::accessor<%s, "
            "%s>())",
    ...
    result_t operator()(const type::Tuple& n, position_t p) {
        ...
        for ( const auto&& [i, e] : util::enumerate(n.elements()) )
            elems.push_back(
                fmt("::hilti::rt::type_info::tuple::Element{ \"%s\", %s, hilti::rt::tuple::elementOffset<%s, "
                    "%d>() }",
    ...
    result_t operator()(const type::StrongReference& n) {
        return fmt(
            "::hilti::rt::type_info::StrongReference(%s, "
            "::hilti::rt::type_info::StrongReference::accessor<%s>())",
            cg->typeInfo(n.dereferencedType()), cg->compile(n.dereferencedType(), codegen::TypeUsage::Storage));
    }

    result_t operator()(const type::ValueReference& n) {
        return fmt(
            "::hilti::rt::type_info::ValueReference(%s, "
            "::hilti::rt::type_info::ValueReference::accessor<%s>())",
            cg->typeInfo(n.dereferencedType()), cg->compile(n.dereferencedType(), codegen::TypeUsage::Storage));
    }
    ...
    result_t operator()(const type::vector::Iterator& n) {
        ...
        return fmt(
            "::hilti::rt::type_info::VectorIterator(%s, "
            "::hilti::rt::type_info::VectorIterator::accessor<%s%s>())",
...
std::optional<cxx::declaration::Type> CodeGen::typeDeclaration(const hilti::Type& t) {
    // FIXME(bbannier): actually walk the type, needs type-erased `Type`.
    // return VisitorDeclaration2(this, &_cache_types_declarations).dispatch(t);
