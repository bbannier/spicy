...
struct Visitor : public visitor::PostOrder<void, Visitor> {
    ...
    void operator()(const expression::ListComprehension& e, position_t p) {
        ...
        if ( ! type::isResolved(e.local().type()) && type::isResolved(e.input().type()) ) {
            ...
            const auto& et = dynamic_cast<const type::trait::isIterable&>(container).elementType();
            logChange(p.node, et);
    ...
    void operator()(const statement::For& u, position_t p) {
        ...
        const auto& et = dynamic_cast<const type::trait::isDereferenceable&>(
                             dynamic_cast<const type::trait::isIterable&>(t).iteratorType(true))
                             .dereferencedType();
        logChange(p.node, et);
