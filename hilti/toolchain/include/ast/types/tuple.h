...
public:
    Tuple(std::vector<Type> t, Meta m = Meta())
        : Type(nodes(_typesToElements(std::move(t))), std::move(m)), trait::isAllocable(&_traits()) {}
    Tuple(std::vector<tuple::Element> e, Meta m = Meta())
        : Type(nodes(std::move(e)), std::move(m)), trait::isAllocable(&_traits()) {}
    Tuple(Wildcard /*unused*/, Meta m = Meta()) : Type(std::move(m)), trait::isAllocable(&_traits()), _wildcard(true) {}
