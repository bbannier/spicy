...
DispatchResult<Result> do_dispatch_one(Erased& n, size_t ti, Dispatcher& d, typename Iterator::Position& i,
                                       bool& no_match_so_far) { // NOLINT
    static const auto th = std::hash<std::string>()(typeid(Type).name());
    if ( ti != th )
        return {};
