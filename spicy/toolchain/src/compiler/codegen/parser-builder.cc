...
struct ProductionVisitor
    ...
    std::optional<Expression> preParseField(const Production& /* i */, const production::Meta& meta) {
        ...
        if ( field && field->isContainer() ) {
            pre_container_offsets = builder()->addTmp("pre_container_offset",
                                                      builder::tuple({builder::member(state().self, "__begin"),
    ...
    void postParseField(const Production& p, const production::Meta& meta,
        ...
        if ( pre_container_offsets ) {
            prev = builder()->addTmp("prev", builder::tuple({builder::member(state().self, "__begin"),
                                                             builder::member(state().self, "__position")}));
            builder()->addAssign(builder::member(state().self, "__begin"), builder::index(*pre_container_offsets, 0));
            builder()->addAssign(builder::member(state().self, "__position"),
                                 builder::index(*pre_container_offsets, 1));
        }
